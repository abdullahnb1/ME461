...
class CommunicationThread(threading.Thread):
    """Handles all serial communication in a separate thread."""
...
class PygameClient:
    def __init__(self):
        pygame.init()
        pygame.display.set_caption("Pico Tetris - 2 Player (USB Mode)")
    ...
    def draw_error(self):
        ...
        text3 = self.font_small.render(f"Check Pico connection and SERIAL_PORT ({SERIAL_PORT}).", True, COLOR_WHITE)
...
if __name__ == "__main__":
    print("--- PC Tetris Client (USB Mode) ---")
    print(f"Ensure your Pico is connected to: {SERIAL_PORT}")
    print("Run 'pico_tetris_usb.py' on the Pico FIRST.")
    print("-" * 26)
